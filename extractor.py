...
class ParseAgreement:
    ...
    def get_covenant_categories(self, filename):
        ...
        for index, i in enumerate(self.content):
            if not 'covenant' in i.lower():
                continue
            print(i, self.content[index-1].lower(), self.content[index-2].lower(), self.content[index-3].lower())
# Look for section number upwards...
            if (sum(c.isdigit() for c in i) and len([c for c in i if c.isdigit()]) <= 3) or "article" in " ".join([self.content[index-1].lower(), self.content[index-2].lower(), self.content[index-3].lower()]) or "section" in self.content[index-1].lower():
                headings_mapping[i]= []
                print('h', i)
                input()
                if 'article' in i:
